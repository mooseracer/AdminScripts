#https://github.com/AndrewG-1234/PRTG
...
$auth = "username=api_access&passhash=12345"
$PRTGHost = "prtg.mycorp.com"
...
function Get-prtgDevicesInGroup ([string]$StartingID=0)
{
        ...
        $url = "https://$PRTGHost/api/table.xml?content=devices&output=csvtable&columns=objid,probe,groupid,device,host,downsens,partialdownsens,downacksens,upsens,warnsens,pausedsens,unusualsens,undefinedsens,tags,comments&id=$StartingID&count=2500&$auth"
        $request = Invoke-WebRequest -Uri $url -MaximumRedirection 0 -ErrorAction Ignore
...
function Get-prtgSensorChannels ([string]$SensorID)
{
    ...
    $url = "https://$PRTGHost/api/table.xml?content=channels&output=csvtable&columns=name,lastvalue_&id=$SensorID&$auth"
    $request = Invoke-WebRequest -Uri $url -MaximumRedirection 0 -ErrorAction Ignore
    convertFrom-csv ($request) -WarningAction SilentlyContinue
...
function Get-prtgSensorChannelIDs ([string]$SensorID)
{
    ...
    $url = "https://$PRTGHost/controls/channeledit.htm?_hjax=true&id=$SensorID&$auth"
...
function Get-prtgSensorData ([string]$SensorID,[datetime]$StartDate,[datetime]$EndDate)
{
    ...
    $url = "https://$PRTGHost/api/historicdata.csv?id=$SensorID&avg=3600&sdate=$StartDate2&edate=$EndDate2&$auth"
    #$url
    $request = Invoke-WebRequest -Uri $url -MaximumRedirection 0 -ErrorAction Ignore
    convertFrom-csv ($request) -WarningAction SilentlyContinue
...
{
    $url = "https://$PRTGHost/api/getobjectproperty.htm?id=$DeviceID&name=$PropertyName&$auth"
...
function Set-PRTGChannelSettings{
    ...
    $url = "https://$PRTGHost/editsettings?$Property" + "_" + $ChannelID + "=" + "$value&id=$DeviceID&$auth"
    #$url
    $request = Invoke-WebRequest -Uri $url -MaximumRedirection 0 -ErrorAction Ignore
}
...
function Get-PRTGChannelSettings{
    ...
    $url = "https://$PRTGHost/controls/channeledit.htm?_hjax=true&id=$DeviceID&channel=$ChannelID&$auth"
...
function Set-PRTGObjectPause{
    ...
    $url = "https://$PRTGHost/api/pauseobjectfor.htm?id=$ObjectID&duration=$minutes&pausemsg=$Message&$auth"
    $request = Invoke-WebRequest -Uri $url -MaximumRedirection 0 -ErrorAction Ignore
...
function Set-PRTGObjectResume{
    ...
    $ObjectID
		...
		#Resume monitoring for the new sensor:
        $url = "https://$PRTGHost/api/pause.htm?id=$ObjectID&action=1&$auth"
        $request = Invoke-WebRequest -Uri $url -MaximumRedirection 0 -ErrorAction Ignore
...
function Set-PRTGResumeThisServer{
    ...
    foreach ($ip in $MYIPs){
        ...
        foreach ($x in $q){
            $url = "https://$PRTGHost/api/pause.htm?id=" + $x.ID + "&action=1&$auth"
            $request = Invoke-WebRequest -Uri $url -MaximumRedirection 0 -ErrorAction Ignore
        }
    }
}
