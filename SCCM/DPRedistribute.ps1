...
$failures = Get-WmiObject -ComputerName $SiteServer -Namespace root\sms\site_$SiteCode -Query "SELECT * FROM SMS_PackageStatusDistPointsSummarizer WHERE State = 1 or State = 2 Or state = 3"
if ($failures -eq $null)
{
...
else
{
    ...
    $DistributionPoints = Get-WmiObject -ComputerName $SiteServer -Namespace root\sms\site_$SiteCode -Query "SELECT * FROM SMS_DistributionPoint WHERE SiteCode='$($failure.SiteCode)' AND  PackageID='$($failure.PackageID)'"
